#!/usr/bin/env python3
"""Run Phase 3.1 discovery and build the Phase 3.2 repository index."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Discover repository assets and build Scenario relationships."
    )
    parser.add_argument(
        "--repository-root",
...
def main() -> int:
    ...
    payload = {
        ...
        "repository_index": index.to_dict(),
    }
    ...
    if args.output:
        ...
        print(f"Repository index written to {output_path}")
    else:
        print(rendered)

    total_issues = len(discovery.issues) + len(index.issues)
    print(
        f"Indexed {len(index.scenarios)} scenarios and "
