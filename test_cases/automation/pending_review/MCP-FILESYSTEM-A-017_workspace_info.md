---
automation_test_id: MCP-FILESYSTEM-A-017
business_scenario_id: MCP-FILESYSTEM-017
name: Retrieve Filesystem Workspace Information
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-FILESYSTEM-017
tags:
  - regression
  - mcp
  - filesystem
  - workspace-info
script_path: automation/playwright/tests/mcp/filesystem/test_workspace_info.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Filesystem Workspace Information
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-FILESYSTEM-017 |
| Business Scenario Name | Retrieve Filesystem Workspace Information |
| Coverage | Primary successful path for `mcp-filesystem-workspace-info` |
...
Automates positive retrieval of workspace metadata. Administrative workspace changes and inaccessible configuration are excluded.
...
|---|---|
| Expected workspace root or name | Read from environment configuration |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Invoke workspace-info. | The invocation succeeds and returns metadata. |
| 2 | Normalise returned path values when needed. | Equivalent path formats are comparable. |
| 3 | Validate workspace identity. | The configured expected root or name is present. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/filesystem/test_workspace_info.py`
- Add traceability markers for `MCP-FILESYSTEM-A-017` and `MCP-FILESYSTEM-017`.
...
@pytest.mark.automation_id("MCP-FILESYSTEM-A-017")
@pytest.mark.business_scenario_id("MCP-FILESYSTEM-017")
def test_workspace_info(
    ...
    run_id = f"mcp-filesystem-a-017-{int(time.time())}"
    disposable_path = PurePosixPath(test_root) / run_id
    test_data = filesystem_test_data["workspace_info"]
    ...
    # Select or confirm `mcp-filesystem-workspace-info`.
...
- pytest result associated with `MCP-FILESYSTEM-A-017`
...
- Execution metadata associated with `MCP-FILESYSTEM-017` and `MCP-FILESYSTEM-A-017`
