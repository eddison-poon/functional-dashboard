---
automation_test_id: MCP-JIRA-A-025
business_scenario_id: MCP-JIRA-025
name: Retrieve Jira Service Desk by ID
status: DRAFT
priority: HIGH
...
script_path: automation/playwright/tests/mcp/jira/test_get_service_desk_by_id.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Jira Service Desk by ID
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-JIRA-025 |
| Business Scenario Name | Retrieve Jira Service Desk by ID |
| Coverage | Primary successful path for `mcp-jira-jira-get-service-desk-by-id` |
...
Automates retrieval of one known service desk. Invalid IDs and inaccessible desks are excluded.
...
| Expected project key/name | Read from test configuration |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Invoke get-service-desk-by-id. | The invocation succeeds. |
| 2 | Inspect the returned identifier. | It matches the configured service desk ID. |
| 3 | Validate service-desk metadata. | The expected project key or name is returned. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/jira/test_get_service_desk_by_id.py`
- Add traceability markers for `MCP-JIRA-A-025` and `MCP-JIRA-025`.
...
@pytest.mark.mcp
@pytest.mark.jira
@pytest.mark.automation_id("MCP-JIRA-A-025")
@pytest.mark.business_scenario_id("MCP-JIRA-025")
def test_get_service_desk_by_id(
    ...
    # Arrange
    page.goto(base_url)
    test_data = jira_test_data["get_service_desk_by_id"]

    # Act
    # Open the configured agent.
    # Select or confirm `mcp-jira-jira-get-service-desk-by-id`.
...
- pytest result associated with `MCP-JIRA-A-025`
...
- Execution metadata associated with `MCP-JIRA-025` and `MCP-JIRA-A-025`
