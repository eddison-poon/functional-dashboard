---
automation_test_id: MCP-GITHUB-A-019
business_scenario_id: MCP-GITHUB-019
name: Retrieve GitHub Pull Request
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - get-pull-request
script_path: automation/playwright/tests/mcp/github/test_get_pull_request.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve GitHub Pull Request

## Objective

Verify automatically that an authorised caller can retrieve pull request details for an existing pull request, and that the returned number, title and branch information match expectations.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-019 |
| Business Scenario Name | Retrieve GitHub Pull Request |
| Coverage | Primary successful path for `mcp-github-get-pull-request` |
...
Automates the primary successful path for `mcp-github-get-pull-request`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- The `mcp-github-get-pull-request` tool is available in the GitHub MCP server.
...
|---:|---|---|
| 1 | Submit the valid identifiers required to retrieve pull request details. | The request is accepted without validation or permission errors. |
| 2 | Retrieve the tool response. | A non-error object or collection is returned. |
| 3 | Inspect the returned data. | The returned number, title and branch information match expectations. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_get_pull_request.py`
- Add traceability markers for `MCP-GITHUB-A-019` and `MCP-GITHUB-019`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-019")
@pytest.mark.business_scenario_id("MCP-GITHUB-019")
def test_get_pull_request(
    ...
    run_id = f"mcp-github-a-019-{int(time.time())}"
    test_data = github_test_data["get_pull_request"]
    ...
    # Select or confirm `mcp-github-get-pull-request`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the returned number, title and branch information match expectations.
...
- pytest result associated with `MCP-GITHUB-A-019`
...
- Execution metadata associated with `MCP-GITHUB-019` and `MCP-GITHUB-A-019`
