---
automation_test_id: MCP-CONFLUENCE-A-007
business_scenario_id: MCP-CONFLUENCE-007
name: Retrieve Comments from Confluence Page
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-CONFLUENCE-007
tags:
  - regression
  - mcp
  - confluence
  - get-comments
script_path: automation/playwright/tests/mcp/confluence/test_get_comments.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Comments from Confluence Page
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-CONFLUENCE-007 |
| Business Scenario Name | Retrieve Comments from Confluence Page |
| Coverage | Primary successful path for `mcp-confluence-get-comments` |
...
Automates positive comment retrieval. Pagination, deleted comments and permission failures are excluded.
...
|---|---|
| Page reference | Create or configure a page |
| Comment marker | Generate and add a known comment during setup |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Prepare a page with a known comment. | The comment exists. |
| 2 | Invoke get-comments. | The invocation succeeds. |
| 3 | Inspect results. | The known marker is present. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/confluence/test_get_comments.py`
- Add traceability markers for `MCP-CONFLUENCE-A-007` and `MCP-CONFLUENCE-007`.
...
@pytest.mark.automation_id("MCP-CONFLUENCE-A-007")
@pytest.mark.business_scenario_id("MCP-CONFLUENCE-007")
def test_get_comments(
    ...
    run_id = f"mcp-confluence-a-007-{int(time.time())}"
    test_data = confluence_test_data["get_comments"]
    ...
    # Select or confirm `mcp-confluence-get-comments`.
...
- Delete the disposable page or comment when supported.

## Expected Execution Outputs

- pytest result associated with `MCP-CONFLUENCE-A-007`
...
- Execution metadata associated with `MCP-CONFLUENCE-007` and `MCP-CONFLUENCE-A-007`
