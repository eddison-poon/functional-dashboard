---
automation_test_id: MCP-GITHUB-A-018
business_scenario_id: MCP-GITHUB-018
name: Retrieve GitHub Organization Details
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - get-org-details
script_path: automation/playwright/tests/mcp/github/test_get_org_details.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve GitHub Organization Details

## Objective

Verify automatically that an authorised caller can retrieve organization metadata for an accessible GitHub organization, and that the returned login or name matches the target organization.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-018 |
| Business Scenario Name | Retrieve GitHub Organization Details |
| Coverage | Primary successful path for `mcp-github-get-org-details` |
...
Automates the primary successful path for `mcp-github-get-org-details`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_get_org_details.py`
- Add traceability markers for `MCP-GITHUB-A-018` and `MCP-GITHUB-018`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-018")
@pytest.mark.business_scenario_id("MCP-GITHUB-018")
def test_get_org_details(
    ...
    run_id = f"mcp-github-a-018-{int(time.time())}"
    test_data = github_test_data["get_org_details"]
    ...
    # Select or confirm `mcp-github-get-org-details`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the returned login or name matches the target organization.
...
- pytest result associated with `MCP-GITHUB-A-018`
...
- Execution metadata associated with `MCP-GITHUB-018` and `MCP-GITHUB-A-018`
