---
automation_test_id: MCP-GITHUB-A-032
business_scenario_id: MCP-GITHUB-032
...
tags:
  ...
  - github
  - list-pull-requests
script_path: automation/playwright/tests/mcp/github/test_list_pull_requests.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List GitHub Pull Requests

## Objective

Verify automatically that an authorised caller can list pull requests for an existing repository with a known pull request, and that the expected pull request number is included.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-032 |
| Business Scenario Name | List GitHub Pull Requests |
| Coverage | Primary successful path for `mcp-github-list-pull-requests` |
...
Automates the primary successful path for `mcp-github-list-pull-requests`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_list_pull_requests.py`
- Add traceability markers for `MCP-GITHUB-A-032` and `MCP-GITHUB-032`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-032")
@pytest.mark.business_scenario_id("MCP-GITHUB-032")
def test_list_pull_requests(
    ...
    run_id = f"mcp-github-a-032-{int(time.time())}"
    test_data = github_test_data["list_pull_requests"]
    ...
    # Select or confirm `mcp-github-list-pull-requests`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the expected pull request number is included.
...
- pytest result associated with `MCP-GITHUB-A-032`
...
- Execution metadata associated with `MCP-GITHUB-032` and `MCP-GITHUB-A-032`
