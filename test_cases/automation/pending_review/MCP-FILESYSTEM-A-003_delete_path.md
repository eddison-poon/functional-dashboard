---
automation_test_id: MCP-FILESYSTEM-A-003
business_scenario_id: MCP-FILESYSTEM-003
name: Delete File or Directory from Allowed Path
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-FILESYSTEM-003
tags:
  - regression
  - mcp
  - filesystem
  - delete-path
script_path: automation/playwright/tests/mcp/filesystem/test_delete_path.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Delete File or Directory from Allowed Path
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-FILESYSTEM-003 |
| Business Scenario Name | Delete File or Directory from Allowed Path |
| Coverage | Primary successful path for `mcp-filesystem-delete-path` |
...
Automates deletion of a disposable file or empty directory. Protected paths, recursive deletion and permission failures are excluded.
...
|---|---|
| Target path | Create a disposable file during setup and retain its path |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Create a disposable target file. | The file exists. |
| 2 | Invoke delete-path with the target path. | The invocation succeeds. |
| 3 | Invoke get-file-info or list-directory. | The deleted path is reported missing or absent. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/filesystem/test_delete_path.py`
- Add traceability markers for `MCP-FILESYSTEM-A-003` and `MCP-FILESYSTEM-003`.
...
@pytest.mark.automation_id("MCP-FILESYSTEM-A-003")
@pytest.mark.business_scenario_id("MCP-FILESYSTEM-003")
def test_delete_path(
    ...
    run_id = f"mcp-filesystem-a-003-{int(time.time())}"
    disposable_path = PurePosixPath(test_root) / run_id
    test_data = filesystem_test_data["delete_path"]
    ...
    # Select or confirm `mcp-filesystem-delete-path`.
...
- pytest result associated with `MCP-FILESYSTEM-A-003`
...
- Execution metadata associated with `MCP-FILESYSTEM-003` and `MCP-FILESYSTEM-A-003`
