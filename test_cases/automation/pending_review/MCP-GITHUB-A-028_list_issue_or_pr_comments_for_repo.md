---
automation_test_id: MCP-GITHUB-A-028
business_scenario_id: MCP-GITHUB-028
name: List GitHub Repository Issue and Pull Request Comments
status: DRAFT
priority: HIGH
...
tags:
  ...
  - mcp
  - github
  - list-issue-or-pr-comments-for-repo
script_path: automation/playwright/tests/mcp/github/test_list_issue_or_pr_comments_for_repo.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List GitHub Repository Issue and Pull Request Comments

## Objective

Verify automatically that an authorised caller can list repository comments for a repository containing a known issue or pull request comment, and that the known comment is included.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-028 |
| Business Scenario Name | List GitHub Repository Issue and Pull Request Comments |
| Coverage | Primary successful path for `mcp-github-list-issue-or-pr-comments-for-repo` |
...
Automates the primary successful path for `mcp-github-list-issue-or-pr-comments-for-repo`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- The target a repository containing a known issue or pull request comment exists and is accessible.
- The `mcp-github-list-issue-or-pr-comments-for-repo` tool is available in the GitHub MCP server.
...
|---:|---|---|
| 1 | Submit the valid identifiers required to list repository comments. | The request is accepted without validation or permission errors. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_list_issue_or_pr_comments_for_repo.py`
- Add traceability markers for `MCP-GITHUB-A-028` and `MCP-GITHUB-028`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-028")
@pytest.mark.business_scenario_id("MCP-GITHUB-028")
def test_list_issue_or_pr_comments_for_repo(
    page: Page,
    ...
    run_id = f"mcp-github-a-028-{int(time.time())}"
    test_data = github_test_data["list_issue_or_pr_comments_for_repo"]
    ...
    # Select or confirm `mcp-github-list-issue-or-pr-comments-for-repo`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the known comment is included.
...
- pytest result associated with `MCP-GITHUB-A-028`
...
- Execution metadata associated with `MCP-GITHUB-028` and `MCP-GITHUB-A-028`
