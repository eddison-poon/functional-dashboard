---
automation_test_id: MCP-GITHUB-A-003
business_scenario_id: MCP-GITHUB-003
...
tags:
  ...
  - github
  - create-pull-request
script_path: automation/playwright/tests/mcp/github/test_create_pull_request.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Create GitHub Pull Request

## Objective

Verify automatically that an authorised caller can create a pull request for a repository with distinct source and target branches, and that the pull request is returned with the expected title, branches and open state.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-003 |
| Business Scenario Name | Create GitHub Pull Request |
| Coverage | Primary successful path for `mcp-github-create-pull-request` |
...
Automates the primary successful path for `mcp-github-create-pull-request`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_create_pull_request.py`
- Add traceability markers for `MCP-GITHUB-A-003` and `MCP-GITHUB-003`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-003")
@pytest.mark.business_scenario_id("MCP-GITHUB-003")
def test_create_pull_request(
    ...
    run_id = f"mcp-github-a-003-{int(time.time())}"
    test_data = github_test_data["create_pull_request"]
    ...
    # Select or confirm `mcp-github-create-pull-request`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the pull request is returned with the expected title, branches and open state.
...
- pytest result associated with `MCP-GITHUB-A-003`
...
- Execution metadata associated with `MCP-GITHUB-003` and `MCP-GITHUB-A-003`
