---
automation_test_id: MCP-GITHUB-A-030
business_scenario_id: MCP-GITHUB-030
name: List GitHub Organizations
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - list-organizations
script_path: automation/playwright/tests/mcp/github/test_list_organizations.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List GitHub Organizations

## Objective

Verify automatically that an authorised caller can list organizations for the accessible GitHub account context, and that the expected organization is included.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-030 |
| Business Scenario Name | List GitHub Organizations |
| Coverage | Primary successful path for `mcp-github-list-organizations` |
...
Automates the primary successful path for `mcp-github-list-organizations`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_list_organizations.py`
- Add traceability markers for `MCP-GITHUB-A-030` and `MCP-GITHUB-030`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-030")
@pytest.mark.business_scenario_id("MCP-GITHUB-030")
def test_list_organizations(
    ...
    run_id = f"mcp-github-a-030-{int(time.time())}"
    test_data = github_test_data["list_organizations"]
    ...
    # Select or confirm `mcp-github-list-organizations`.
...
- pytest result associated with `MCP-GITHUB-A-030`
...
- Execution metadata associated with `MCP-GITHUB-030` and `MCP-GITHUB-A-030`
