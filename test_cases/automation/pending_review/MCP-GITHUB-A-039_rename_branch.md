---
automation_test_id: MCP-GITHUB-A-039
business_scenario_id: MCP-GITHUB-039
name: Rename GitHub Branch
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - rename-branch
script_path: automation/playwright/tests/mcp/github/test_rename_branch.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Rename GitHub Branch

## Objective

Verify automatically that an authorised caller can rename a branch for a disposable non-protected branch, and that the new branch name is returned and the old name is absent.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-039 |
| Business Scenario Name | Rename GitHub Branch |
| Coverage | Primary successful path for `mcp-github-rename-branch` |
...
Automates the primary successful path for `mcp-github-rename-branch`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- A disposable or approved test a disposable non-protected branch exists.
- The current state and intended updated state are known.
- The `mcp-github-rename-branch` tool is available in the GitHub MCP server.
...
| Updated value | Generate a valid value containing `MCP-GITHUB-A-039` and the execution timestamp |
...
|---:|---|---|
| 1 | Submit valid identifiers and updated values to rename a branch. | The request is accepted. |
| 2 | Confirm the state-changing operation. | The tool reports successful completion. |
| 3 | Retrieve the updated GitHub resource. | The new branch name is returned and the old name is absent. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_rename_branch.py`
- Add traceability markers for `MCP-GITHUB-A-039` and `MCP-GITHUB-039`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-039")
@pytest.mark.business_scenario_id("MCP-GITHUB-039")
def test_rename_branch(
    ...
    run_id = f"mcp-github-a-039-{int(time.time())}"
    test_data = github_test_data["rename_branch"]
    ...
    # Select or confirm `mcp-github-rename-branch`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the new branch name is returned and the old name is absent.
...
- pytest result associated with `MCP-GITHUB-A-039`
...
- Execution metadata associated with `MCP-GITHUB-039` and `MCP-GITHUB-A-039`
