---
automation_test_id: MCP-GITHUB-A-002
business_scenario_id: MCP-GITHUB-002
name: Create GitHub Organization Repository
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - create-org-repo
script_path: automation/playwright/tests/mcp/github/test_create_org_repo.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Create GitHub Organization Repository

## Objective

Verify automatically that an authorised caller can create a repository for a target GitHub organization, and that the repository exists under the organization with the submitted settings.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-002 |
| Business Scenario Name | Create GitHub Organization Repository |
| Coverage | Primary successful path for `mcp-github-create-org-repo` |
...
Automates the primary successful path for `mcp-github-create-org-repo`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_create_org_repo.py`
- Add traceability markers for `MCP-GITHUB-A-002` and `MCP-GITHUB-002`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-002")
@pytest.mark.business_scenario_id("MCP-GITHUB-002")
def test_create_org_repo(
    ...
    run_id = f"mcp-github-a-002-{int(time.time())}"
    test_data = github_test_data["create_org_repo"]
    ...
    # Select or confirm `mcp-github-create-org-repo`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the repository exists under the organization with the submitted settings.
...
- pytest result associated with `MCP-GITHUB-A-002`
...
- Execution metadata associated with `MCP-GITHUB-002` and `MCP-GITHUB-A-002`
