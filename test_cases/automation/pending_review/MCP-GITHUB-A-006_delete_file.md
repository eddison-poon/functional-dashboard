---
automation_test_id: MCP-GITHUB-A-006
business_scenario_id: MCP-GITHUB-006
name: Delete File from GitHub Repository
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - delete-file
script_path: automation/playwright/tests/mcp/github/test_delete_file.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Delete File from GitHub Repository

## Objective

Verify automatically that an authorised caller can delete a file for an existing file in a disposable repository branch, and that the file is no longer returned at the target path.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-006 |
| Business Scenario Name | Delete File from GitHub Repository |
| Coverage | Primary successful path for `mcp-github-delete-file` |
...
Automates the primary successful path for `mcp-github-delete-file`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_delete_file.py`
- Add traceability markers for `MCP-GITHUB-A-006` and `MCP-GITHUB-006`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-006")
@pytest.mark.business_scenario_id("MCP-GITHUB-006")
def test_delete_file(
    ...
    run_id = f"mcp-github-a-006-{int(time.time())}"
    test_data = github_test_data["delete_file"]
    ...
    # Select or confirm `mcp-github-delete-file`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the file is no longer returned at the target path.
...
- pytest result associated with `MCP-GITHUB-A-006`
...
- Execution metadata associated with `MCP-GITHUB-006` and `MCP-GITHUB-A-006`
