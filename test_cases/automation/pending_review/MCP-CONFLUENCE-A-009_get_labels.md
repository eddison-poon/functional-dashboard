---
automation_test_id: MCP-CONFLUENCE-A-009
business_scenario_id: MCP-CONFLUENCE-009
name: Retrieve Labels from Confluence Page
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-CONFLUENCE-009
tags:
  - regression
  - mcp
  - confluence
  - get-labels
script_path: automation/playwright/tests/mcp/confluence/test_get_labels.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Labels from Confluence Page
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-CONFLUENCE-009 |
| Business Scenario Name | Retrieve Labels from Confluence Page |
| Coverage | Primary successful path for `mcp-confluence-get-labels` |
...
Automates positive label retrieval. Pagination and permission failures are excluded.
...
| Expected label | Generate and add a known label during setup |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Prepare a page with a known label. | The label exists. |
| 2 | Invoke get-labels. | The invocation succeeds. |
| 3 | Inspect results. | The expected label is present. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/confluence/test_get_labels.py`
- Add traceability markers for `MCP-CONFLUENCE-A-009` and `MCP-CONFLUENCE-009`.
...
@pytest.mark.automation_id("MCP-CONFLUENCE-A-009")
@pytest.mark.business_scenario_id("MCP-CONFLUENCE-009")
def test_get_labels(
    page: Page,
    ...
    run_id = f"mcp-confluence-a-009-{int(time.time())}"
    test_data = confluence_test_data["get_labels"]
    ...
    # Select or confirm `mcp-confluence-get-labels`.
...
- Delete the disposable page or remove the label when supported.

## Expected Execution Outputs

- pytest result associated with `MCP-CONFLUENCE-A-009`
...
- Execution metadata associated with `MCP-CONFLUENCE-009` and `MCP-CONFLUENCE-A-009`
