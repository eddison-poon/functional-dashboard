---
automation_test_id: MCP-JIRA-A-018
business_scenario_id: MCP-JIRA-018
name: Retrieve Jira Custom Fields
status: DRAFT
priority: HIGH
...
tags:
  ...
  - jira
  - get-custom-fields
script_path: automation/playwright/tests/mcp/jira/test_get_custom_fields.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Jira Custom Fields
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-JIRA-018 |
| Business Scenario Name | Retrieve Jira Custom Fields |
| Coverage | Primary successful path for `mcp-jira-jira-get-custom-fields` |
...
Automates positive retrieval of available custom fields. Permission-hidden fields, schema completeness and pagination are excluded.
...
|---|---|
| Expected field ID or name | Read from test configuration |
...
|---:|---|---|
| 1 | Ensure the get-custom-fields tool is available. | The tool is listed and selectable. |
| 2 | Invoke the tool. | The invocation succeeds and returns a field collection. |
| 3 | Inspect the collection. | The configured expected custom field is present. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/jira/test_get_custom_fields.py`
- Include markers or metadata for `MCP-JIRA-A-018` and `MCP-JIRA-018`.
...
@pytest.mark.mcp
@pytest.mark.jira
@pytest.mark.automation_id("MCP-JIRA-A-018")
@pytest.mark.business_scenario_id("MCP-JIRA-018")
def test_get_custom_fields(page: Page) -> None:
    base_url = os.getenv("BASE_URL")
    if not base_url:
        pytest.fail("BASE_URL is required")

    unique_value = f"MCP-JIRA-A-018-{int(time.time())}"

    # Arrange
    page.goto(base_url)

    # Act
    # Open the configured agent and invoke `mcp-jira-jira-get-custom-fields`.
...
- pytest result associated with `MCP-JIRA-A-018`
...
- Execution metadata associated with `MCP-JIRA-018` and `MCP-JIRA-A-018`
