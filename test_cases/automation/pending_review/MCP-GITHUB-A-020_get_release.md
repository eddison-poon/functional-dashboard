---
automation_test_id: MCP-GITHUB-A-020
business_scenario_id: MCP-GITHUB-020
name: Retrieve GitHub Release by ID
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - get-release
script_path: automation/playwright/tests/mcp/github/test_get_release.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve GitHub Release by ID

## Objective

Verify automatically that an authorised caller can retrieve a release for an existing release identifier, and that the returned release ID and tag match the target release.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-020 |
| Business Scenario Name | Retrieve GitHub Release by ID |
| Coverage | Primary successful path for `mcp-github-get-release` |
...
Automates the primary successful path for `mcp-github-get-release`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_get_release.py`
- Add traceability markers for `MCP-GITHUB-A-020` and `MCP-GITHUB-020`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-020")
@pytest.mark.business_scenario_id("MCP-GITHUB-020")
def test_get_release(
    ...
    run_id = f"mcp-github-a-020-{int(time.time())}"
    test_data = github_test_data["get_release"]
    ...
    # Select or confirm `mcp-github-get-release`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the returned release ID and tag match the target release.
...
- pytest result associated with `MCP-GITHUB-A-020`
...
- Execution metadata associated with `MCP-GITHUB-020` and `MCP-GITHUB-A-020`
