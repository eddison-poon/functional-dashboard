---
automation_test_id: MCP-GITHUB-A-017
business_scenario_id: MCP-GITHUB-017
name: Retrieve Latest GitHub Release
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - get-latest-release
script_path: automation/playwright/tests/mcp/github/test_get_latest_release.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Latest GitHub Release

## Objective

Verify automatically that an authorised caller can retrieve the latest release for a repository with a known latest release, and that the returned release tag and identifier match the latest release.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-017 |
| Business Scenario Name | Retrieve Latest GitHub Release |
| Coverage | Primary successful path for `mcp-github-get-latest-release` |
...
Automates the primary successful path for `mcp-github-get-latest-release`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_get_latest_release.py`
- Add traceability markers for `MCP-GITHUB-A-017` and `MCP-GITHUB-017`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-017")
@pytest.mark.business_scenario_id("MCP-GITHUB-017")
def test_get_latest_release(
    ...
    run_id = f"mcp-github-a-017-{int(time.time())}"
    test_data = github_test_data["get_latest_release"]
    ...
    # Select or confirm `mcp-github-get-latest-release`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the returned release tag and identifier match the latest release.
...
- pytest result associated with `MCP-GITHUB-A-017`
...
- Execution metadata associated with `MCP-GITHUB-017` and `MCP-GITHUB-A-017`
