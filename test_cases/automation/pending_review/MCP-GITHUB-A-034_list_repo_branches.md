---
automation_test_id: MCP-GITHUB-A-034
business_scenario_id: MCP-GITHUB-034
name: List GitHub Repository Branches
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - list-repo-branches
script_path: automation/playwright/tests/mcp/github/test_list_repo_branches.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List GitHub Repository Branches

## Objective

Verify automatically that an authorised caller can list branches for an existing repository with a known branch, and that the expected branch is included.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-034 |
| Business Scenario Name | List GitHub Repository Branches |
| Coverage | Primary successful path for `mcp-github-list-repo-branches` |
...
Automates the primary successful path for `mcp-github-list-repo-branches`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_list_repo_branches.py`
- Add traceability markers for `MCP-GITHUB-A-034` and `MCP-GITHUB-034`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-034")
@pytest.mark.business_scenario_id("MCP-GITHUB-034")
def test_list_repo_branches(
    page: Page,
    ...
    run_id = f"mcp-github-a-034-{int(time.time())}"
    test_data = github_test_data["list_repo_branches"]
    ...
    # Select or confirm `mcp-github-list-repo-branches`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the expected branch is included.
...
- pytest result associated with `MCP-GITHUB-A-034`
...
- Execution metadata associated with `MCP-GITHUB-034` and `MCP-GITHUB-A-034`
