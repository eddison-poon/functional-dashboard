---
automation_test_id: MCP-GITHUB-A-046
business_scenario_id: MCP-GITHUB-046
name: Create or Update File in GitHub Repository
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - upsert-file
script_path: automation/playwright/tests/mcp/github/test_upsert_file.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Create or Update File in GitHub Repository

## Objective

Verify automatically that an authorised caller can create or update a file for a writable repository branch, and that the target file exists with the submitted content.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-046 |
| Business Scenario Name | Create or Update File in GitHub Repository |
| Coverage | Primary successful path for `mcp-github-upsert-file` |
...
Automates the primary successful path for `mcp-github-upsert-file`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_upsert_file.py`
- Add traceability markers for `MCP-GITHUB-A-046` and `MCP-GITHUB-046`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-046")
@pytest.mark.business_scenario_id("MCP-GITHUB-046")
def test_upsert_file(
    ...
    run_id = f"mcp-github-a-046-{int(time.time())}"
    test_data = github_test_data["upsert_file"]
    ...
    # Select or confirm `mcp-github-upsert-file`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the target file exists with the submitted content.
...
- pytest result associated with `MCP-GITHUB-A-046`
...
- Execution metadata associated with `MCP-GITHUB-046` and `MCP-GITHUB-A-046`
