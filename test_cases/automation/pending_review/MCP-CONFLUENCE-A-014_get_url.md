---
automation_test_id: MCP-CONFLUENCE-A-014
business_scenario_id: MCP-CONFLUENCE-014
name: Retrieve Confluence Resource URL
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-CONFLUENCE-014
tags:
  - regression
  - mcp
  - confluence
  - get-url
script_path: automation/playwright/tests/mcp/confluence/test_get_url.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Confluence Resource URL
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-CONFLUENCE-014 |
| Business Scenario Name | Retrieve Confluence Resource URL |
| Coverage | Primary successful path for `mcp-confluence-get-url` |
...
Automates URL generation and format validation. Anonymous access and expired links are excluded.
...
|---|---|
| Resource reference | Read from configuration or setup |
| Expected host | Read from `CONFLUENCE_BASE_URL` |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Invoke get-url. | The invocation succeeds. |
| 2 | Validate format. | The URL is well formed. |
| 3 | Validate target. | The expected host and resource marker are present. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/confluence/test_get_url.py`
- Add traceability markers for `MCP-CONFLUENCE-A-014` and `MCP-CONFLUENCE-014`.
...
@pytest.mark.automation_id("MCP-CONFLUENCE-A-014")
@pytest.mark.business_scenario_id("MCP-CONFLUENCE-014")
def test_get_url(
    page: Page,
    ...
    run_id = f"mcp-confluence-a-014-{int(time.time())}"
    test_data = confluence_test_data["get_url"]
    ...
    # Select or confirm `mcp-confluence-get-url`.
...
- pytest result associated with `MCP-CONFLUENCE-A-014`
...
- Execution metadata associated with `MCP-CONFLUENCE-014` and `MCP-CONFLUENCE-A-014`
