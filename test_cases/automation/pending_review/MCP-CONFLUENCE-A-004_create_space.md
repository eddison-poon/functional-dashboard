---
automation_test_id: MCP-CONFLUENCE-A-004
business_scenario_id: MCP-CONFLUENCE-004
name: Create Confluence Space with Valid Details
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-CONFLUENCE-004
tags:
  - regression
  - mcp
  - confluence
  - create-space
script_path: automation/playwright/tests/mcp/confluence/test_create_space.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Create Confluence Space with Valid Details
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-CONFLUENCE-004 |
| Business Scenario Name | Create Confluence Space with Valid Details |
| Coverage | Primary successful path for `mcp-confluence-create-space` |
...
Automates successful creation of a disposable space. Duplicate keys, invalid values and permission failures are excluded.
...
|---|---|
| Space key | Generate a unique uppercase key within configured limits |
| Space name | Generate `MCP Confluence Automation ${timestamp}` |
| Description | Generate an automation marker |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Invoke create-space with generated data. | The invocation succeeds. |
| 2 | Invoke get-space with the returned reference. | The space is returned. |
| 3 | Validate metadata. | Key, name and description match. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/confluence/test_create_space.py`
- Add traceability markers for `MCP-CONFLUENCE-A-004` and `MCP-CONFLUENCE-004`.
...
@pytest.mark.automation_id("MCP-CONFLUENCE-A-004")
@pytest.mark.business_scenario_id("MCP-CONFLUENCE-004")
def test_create_space(
    ...
    run_id = f"mcp-confluence-a-004-{int(time.time())}"
    test_data = confluence_test_data["create_space"]
    ...
    # Select or confirm `mcp-confluence-create-space`.
...
- Delete the created disposable space.

## Expected Execution Outputs

- pytest result associated with `MCP-CONFLUENCE-A-004`
...
- Execution metadata associated with `MCP-CONFLUENCE-004` and `MCP-CONFLUENCE-A-004`
