---
automation_test_id: MCP-GITHUB-A-004
business_scenario_id: MCP-GITHUB-004
...
tags:
  ...
  - github
  - create-reference
script_path: automation/playwright/tests/mcp/github/test_create_reference.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Create Git Reference

## Objective

Verify automatically that an authorised caller can create a Git reference for an existing repository and source commit, and that the new reference resolves to the intended commit.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-004 |
| Business Scenario Name | Create Git Reference |
| Coverage | Primary successful path for `mcp-github-create-reference` |
...
Automates the primary successful path for `mcp-github-create-reference`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_create_reference.py`
- Add traceability markers for `MCP-GITHUB-A-004` and `MCP-GITHUB-004`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-004")
@pytest.mark.business_scenario_id("MCP-GITHUB-004")
def test_create_reference(
    ...
    run_id = f"mcp-github-a-004-{int(time.time())}"
    test_data = github_test_data["create_reference"]
    ...
    # Select or confirm `mcp-github-create-reference`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the new reference resolves to the intended commit.
...
- pytest result associated with `MCP-GITHUB-A-004`
...
- Execution metadata associated with `MCP-GITHUB-004` and `MCP-GITHUB-A-004`
