---
automation_test_id: MCP-GITHUB-A-025
business_scenario_id: MCP-GITHUB-025
name: List GitHub Repository Commits
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - list-commits
script_path: automation/playwright/tests/mcp/github/test_list_commits.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List GitHub Repository Commits

## Objective

Verify automatically that an authorised caller can list commits for an existing repository or branch, and that a known commit SHA is included.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-025 |
| Business Scenario Name | List GitHub Repository Commits |
| Coverage | Primary successful path for `mcp-github-list-commits` |
...
Automates the primary successful path for `mcp-github-list-commits`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- The target an existing repository or branch exists and is accessible.
- The `mcp-github-list-commits` tool is available in the GitHub MCP server.
...
|---:|---|---|
| 1 | Submit the valid identifiers required to list commits. | The request is accepted without validation or permission errors. |
| 2 | Retrieve the tool response. | A non-error object or collection is returned. |
| 3 | Inspect the returned data. | A known commit sha is included. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_list_commits.py`
- Add traceability markers for `MCP-GITHUB-A-025` and `MCP-GITHUB-025`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-025")
@pytest.mark.business_scenario_id("MCP-GITHUB-025")
def test_list_commits(
    ...
    run_id = f"mcp-github-a-025-{int(time.time())}"
    test_data = github_test_data["list_commits"]
...
- pytest result associated with `MCP-GITHUB-A-025`
...
- Execution metadata associated with `MCP-GITHUB-025` and `MCP-GITHUB-A-025`
