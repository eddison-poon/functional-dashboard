---
automation_test_id: MCP-GITHUB-A-044
business_scenario_id: MCP-GITHUB-044
name: Update GitHub Pull Request Branch
status: DRAFT
priority: HIGH
...
script_path: automation/playwright/tests/mcp/github/test_update_pull_request_branch.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Update GitHub Pull Request Branch

## Objective

Verify automatically that an authorised caller can update the pull request branch for an open pull request behind its base branch, and that the pull request head is updated with the latest base changes.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-044 |
| Business Scenario Name | Update GitHub Pull Request Branch |
| Coverage | Primary successful path for `mcp-github-update-pull-request-branch` |
...
Automates the primary successful path for `mcp-github-update-pull-request-branch`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_update_pull_request_branch.py`
- Add traceability markers for `MCP-GITHUB-A-044` and `MCP-GITHUB-044`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-044")
@pytest.mark.business_scenario_id("MCP-GITHUB-044")
def test_update_pull_request_branch(
    ...
    run_id = f"mcp-github-a-044-{int(time.time())}"
    test_data = github_test_data["update_pull_request_branch"]
    ...
    # Select or confirm `mcp-github-update-pull-request-branch`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the pull request head is updated with the latest base changes.
...
- pytest result associated with `MCP-GITHUB-A-044`
...
- Execution metadata associated with `MCP-GITHUB-044` and `MCP-GITHUB-A-044`
