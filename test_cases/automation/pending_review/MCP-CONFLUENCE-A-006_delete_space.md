---
automation_test_id: MCP-CONFLUENCE-A-006
business_scenario_id: MCP-CONFLUENCE-006
name: Delete Existing Confluence Space
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-CONFLUENCE-006
tags:
  - regression
  - mcp
  - confluence
  - delete-space
script_path: automation/playwright/tests/mcp/confluence/test_delete_space.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Delete Existing Confluence Space
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-CONFLUENCE-006 |
| Business Scenario Name | Delete Existing Confluence Space |
| Coverage | Primary successful path for `mcp-confluence-delete-space` |
...
Automates deletion of a disposable space. Protected spaces, non-empty restrictions and permission failures are excluded.
...
|---|---|
| Space reference | Create a disposable space during setup |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Create a disposable space. | A valid space reference is available. |
| 2 | Invoke delete-space. | The invocation succeeds. |
| 3 | Invoke get-space. | The result indicates missing, deleted or inaccessible. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/confluence/test_delete_space.py`
- Add traceability markers for `MCP-CONFLUENCE-A-006` and `MCP-CONFLUENCE-006`.
...
@pytest.mark.automation_id("MCP-CONFLUENCE-A-006")
@pytest.mark.business_scenario_id("MCP-CONFLUENCE-006")
def test_delete_space(
    ...
    run_id = f"mcp-confluence-a-006-{int(time.time())}"
    test_data = confluence_test_data["delete_space"]
    ...
    # Select or confirm `mcp-confluence-delete-space`.
...
- pytest result associated with `MCP-CONFLUENCE-A-006`
...
- Execution metadata associated with `MCP-CONFLUENCE-006` and `MCP-CONFLUENCE-A-006`
