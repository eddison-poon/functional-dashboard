---
automation_test_id: MCP-GITHUB-A-012
business_scenario_id: MCP-GITHUB-012
name: Retrieve GitHub Branch
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - get-branch
script_path: automation/playwright/tests/mcp/github/test_get_branch.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve GitHub Branch

## Objective

Verify automatically that an authorised caller can retrieve branch details for an existing repository branch, and that the returned branch name and head commit match the expected branch.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-012 |
| Business Scenario Name | Retrieve GitHub Branch |
| Coverage | Primary successful path for `mcp-github-get-branch` |
...
Automates the primary successful path for `mcp-github-get-branch`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- The target an existing repository branch exists and is accessible.
- The `mcp-github-get-branch` tool is available in the GitHub MCP server.
...
|---:|---|---|
| 1 | Submit the valid identifiers required to retrieve branch details. | The request is accepted without validation or permission errors. |
| 2 | Retrieve the tool response. | A non-error object or collection is returned. |
| 3 | Inspect the returned data. | The returned branch name and head commit match the expected branch. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_get_branch.py`
- Add traceability markers for `MCP-GITHUB-A-012` and `MCP-GITHUB-012`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-012")
@pytest.mark.business_scenario_id("MCP-GITHUB-012")
def test_get_branch(
    page: Page,
    ...
    run_id = f"mcp-github-a-012-{int(time.time())}"
    test_data = github_test_data["get_branch"]
    ...
    # Select or confirm `mcp-github-get-branch`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the returned branch name and head commit match the expected branch.
...
- pytest result associated with `MCP-GITHUB-A-012`
...
- Execution metadata associated with `MCP-GITHUB-012` and `MCP-GITHUB-A-012`
