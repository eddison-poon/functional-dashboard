---
automation_test_id: MCP-FILESYSTEM-A-005
business_scenario_id: MCP-FILESYSTEM-005
name: Edit Existing Text File
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-FILESYSTEM-005
tags:
  - regression
  - mcp
  - filesystem
  - edit-file
script_path: automation/playwright/tests/mcp/filesystem/test_edit_file.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Edit Existing Text File
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-FILESYSTEM-005 |
| Business Scenario Name | Edit Existing Text File |
| Coverage | Primary successful path for `mcp-filesystem-edit-file` |
...
Automates a single deterministic text edit. Ambiguous replacements, binary files and concurrent modifications are excluded.
...
|---|---|
| Target file | Create a disposable file during setup |
| Original marker | Generate a known source value |
| Replacement marker | Generate `MCP-FILESYSTEM-A-005-${timestamp}` |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Create a disposable file with known original text. | The file contains the source marker. |
| 2 | Invoke edit-file with the replacement instruction. | The invocation succeeds. |
| 3 | Invoke read-text-file. | The replacement marker is present and the original marker is absent. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/filesystem/test_edit_file.py`
- Add traceability markers for `MCP-FILESYSTEM-A-005` and `MCP-FILESYSTEM-005`.
...
@pytest.mark.automation_id("MCP-FILESYSTEM-A-005")
@pytest.mark.business_scenario_id("MCP-FILESYSTEM-005")
def test_edit_file(
    page: Page,
    ...
    run_id = f"mcp-filesystem-a-005-{int(time.time())}"
    disposable_path = PurePosixPath(test_root) / run_id
    test_data = filesystem_test_data["edit_file"]
    ...
    # Select or confirm `mcp-filesystem-edit-file`.
...
- pytest result associated with `MCP-FILESYSTEM-A-005`
...
- Execution metadata associated with `MCP-FILESYSTEM-005` and `MCP-FILESYSTEM-A-005`
