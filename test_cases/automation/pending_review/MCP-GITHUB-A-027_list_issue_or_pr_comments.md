---
automation_test_id: MCP-GITHUB-A-027
business_scenario_id: MCP-GITHUB-027
name: List GitHub Issue or Pull Request Comments
status: DRAFT
priority: HIGH
...
tags:
  ...
  - mcp
  - github
  - list-issue-or-pr-comments
script_path: automation/playwright/tests/mcp/github/test_list_issue_or_pr_comments.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List GitHub Issue or Pull Request Comments

## Objective

Verify automatically that an authorised caller can list comments for an existing issue or pull request with a known comment, and that the known comment is included.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-027 |
| Business Scenario Name | List GitHub Issue or Pull Request Comments |
| Coverage | Primary successful path for `mcp-github-list-issue-or-pr-comments` |
...
Automates the primary successful path for `mcp-github-list-issue-or-pr-comments`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_list_issue_or_pr_comments.py`
- Add traceability markers for `MCP-GITHUB-A-027` and `MCP-GITHUB-027`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-027")
@pytest.mark.business_scenario_id("MCP-GITHUB-027")
def test_list_issue_or_pr_comments(
    page: Page,
    ...
    run_id = f"mcp-github-a-027-{int(time.time())}"
    test_data = github_test_data["list_issue_or_pr_comments"]
    ...
    # Select or confirm `mcp-github-list-issue-or-pr-comments`.
...
- pytest result associated with `MCP-GITHUB-A-027`
...
- Execution metadata associated with `MCP-GITHUB-027` and `MCP-GITHUB-A-027`
