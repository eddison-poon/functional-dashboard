---
automation_test_id: MCP-FILESYSTEM-A-012
business_scenario_id: MCP-FILESYSTEM-012
...
requirement_ids:
  - REQ-MCP-FILESYSTEM-012
tags:
  - regression
  - mcp
  - filesystem
  - read-file
script_path: automation/playwright/tests/mcp/filesystem/test_read_file.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Read Existing File
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-FILESYSTEM-012 |
| Business Scenario Name | Read Existing File |
| Coverage | Primary successful path for `mcp-filesystem-read-file` |
...
Automates reading one small controlled file. Very large files, unsupported encodings and denied paths are excluded.
...
|---|---|
| Target file | Create a disposable supported file |
| Expected marker | Generate a unique known value |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Create a disposable file containing the expected marker. | The file exists. |
| 2 | Invoke read-file. | The invocation succeeds and returns content. |
| 3 | Validate returned data. | The expected marker is present after applying the response format defined by the tool. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/filesystem/test_read_file.py`
- Add traceability markers for `MCP-FILESYSTEM-A-012` and `MCP-FILESYSTEM-012`.
...
@pytest.mark.automation_id("MCP-FILESYSTEM-A-012")
@pytest.mark.business_scenario_id("MCP-FILESYSTEM-012")
def test_read_file(
    page: Page,
    ...
    run_id = f"mcp-filesystem-a-012-{int(time.time())}"
    disposable_path = PurePosixPath(test_root) / run_id
    test_data = filesystem_test_data["read_file"]
    ...
    # Select or confirm `mcp-filesystem-read-file`.
...
- pytest result associated with `MCP-FILESYSTEM-A-012`
...
- Execution metadata associated with `MCP-FILESYSTEM-012` and `MCP-FILESYSTEM-A-012`
