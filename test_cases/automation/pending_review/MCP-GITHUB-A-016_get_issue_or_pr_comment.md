---
automation_test_id: MCP-GITHUB-A-016
business_scenario_id: MCP-GITHUB-016
name: Retrieve GitHub Issue or Pull Request Comment
status: DRAFT
priority: HIGH
...
tags:
  ...
  - mcp
  - github
  - get-issue-or-pr-comment
script_path: automation/playwright/tests/mcp/github/test_get_issue_or_pr_comment.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve GitHub Issue or Pull Request Comment

## Objective

Verify automatically that an authorised caller can retrieve a specific comment for an existing issue or pull request comment, and that the returned comment identifier and body match the expected comment.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-016 |
| Business Scenario Name | Retrieve GitHub Issue or Pull Request Comment |
| Coverage | Primary successful path for `mcp-github-get-issue-or-pr-comment` |
...
Automates the primary successful path for `mcp-github-get-issue-or-pr-comment`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_get_issue_or_pr_comment.py`
- Add traceability markers for `MCP-GITHUB-A-016` and `MCP-GITHUB-016`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-016")
@pytest.mark.business_scenario_id("MCP-GITHUB-016")
def test_get_issue_or_pr_comment(
    page: Page,
    ...
    run_id = f"mcp-github-a-016-{int(time.time())}"
    test_data = github_test_data["get_issue_or_pr_comment"]
    ...
    # Select or confirm `mcp-github-get-issue-or-pr-comment`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the returned comment identifier and body match the expected comment.
...
- pytest result associated with `MCP-GITHUB-A-016`
...
- Execution metadata associated with `MCP-GITHUB-016` and `MCP-GITHUB-A-016`
