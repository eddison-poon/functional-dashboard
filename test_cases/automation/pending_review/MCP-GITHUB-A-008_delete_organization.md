---
automation_test_id: MCP-GITHUB-A-008
business_scenario_id: MCP-GITHUB-008
...
tags:
  ...
  - github
  - delete-organization
script_path: automation/playwright/tests/mcp/github/test_delete_organization.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Delete GitHub Organization

## Objective

Verify automatically that an authorised caller can delete a disposable organization for an organization created for controlled testing, and that the organization is no longer returned as active.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-008 |
| Business Scenario Name | Delete GitHub Organization |
| Coverage | Primary successful path for `mcp-github-delete-organization` |
...
Automates the primary successful path for `mcp-github-delete-organization`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_delete_organization.py`
- Add traceability markers for `MCP-GITHUB-A-008` and `MCP-GITHUB-008`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-008")
@pytest.mark.business_scenario_id("MCP-GITHUB-008")
def test_delete_organization(
    ...
    run_id = f"mcp-github-a-008-{int(time.time())}"
    test_data = github_test_data["delete_organization"]
    ...
    # Select or confirm `mcp-github-delete-organization`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the organization is no longer returned as active.
...
- pytest result associated with `MCP-GITHUB-A-008`
...
- Execution metadata associated with `MCP-GITHUB-008` and `MCP-GITHUB-A-008`
