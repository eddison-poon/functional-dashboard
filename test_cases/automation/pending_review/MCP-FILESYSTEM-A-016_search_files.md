---
automation_test_id: MCP-FILESYSTEM-A-016
business_scenario_id: MCP-FILESYSTEM-016
name: Search Files within Allowed Directory
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-FILESYSTEM-016
tags:
  - regression
  - mcp
  - filesystem
  - search-files
script_path: automation/playwright/tests/mcp/filesystem/test_search_files.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Search Files within Allowed Directory
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-FILESYSTEM-016 |
| Business Scenario Name | Search Files within Allowed Directory |
| Coverage | Primary successful path for `mcp-filesystem-search-files` |
...
Automates one positive search in a controlled directory. Ranking, complex regex behaviour and inaccessible files are excluded.
...
|---|---|
| Search root | Create a disposable directory |
| Matching file | Create a uniquely named or uniquely populated file |
| Search query | Construct a supported query matching the file |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Create a controlled matching file and optional non-matching file. | The files exist. |
| 2 | Invoke search-files with the generated query. | The invocation succeeds and returns matches. |
| 3 | Inspect returned paths. | The expected matching file is present. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/filesystem/test_search_files.py`
- Add traceability markers for `MCP-FILESYSTEM-A-016` and `MCP-FILESYSTEM-016`.
...
@pytest.mark.automation_id("MCP-FILESYSTEM-A-016")
@pytest.mark.business_scenario_id("MCP-FILESYSTEM-016")
def test_search_files(
    ...
    run_id = f"mcp-filesystem-a-016-{int(time.time())}"
    disposable_path = PurePosixPath(test_root) / run_id
    test_data = filesystem_test_data["search_files"]
    ...
    # Select or confirm `mcp-filesystem-search-files`.
...
- pytest result associated with `MCP-FILESYSTEM-A-016`
...
- Execution metadata associated with `MCP-FILESYSTEM-016` and `MCP-FILESYSTEM-A-016`
