---
automation_test_id: MCP-GITHUB-A-042
business_scenario_id: MCP-GITHUB-042
name: Update GitHub Organization
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - update-organization
script_path: automation/playwright/tests/mcp/github/test_update_organization.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Update GitHub Organization

## Objective

Verify automatically that an authorised caller can update supported organization details for a disposable or approved test organization, and that the changed organization fields match the submitted values.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-042 |
| Business Scenario Name | Update GitHub Organization |
| Coverage | Primary successful path for `mcp-github-update-organization` |
...
Automates the primary successful path for `mcp-github-update-organization`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_update_organization.py`
- Add traceability markers for `MCP-GITHUB-A-042` and `MCP-GITHUB-042`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-042")
@pytest.mark.business_scenario_id("MCP-GITHUB-042")
def test_update_organization(
    ...
    run_id = f"mcp-github-a-042-{int(time.time())}"
    test_data = github_test_data["update_organization"]
    ...
    # Select or confirm `mcp-github-update-organization`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the changed organization fields match the submitted values.
...
- pytest result associated with `MCP-GITHUB-A-042`
...
- Execution metadata associated with `MCP-GITHUB-042` and `MCP-GITHUB-A-042`
