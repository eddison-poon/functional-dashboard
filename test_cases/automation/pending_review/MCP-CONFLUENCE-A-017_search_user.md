---
automation_test_id: MCP-CONFLUENCE-A-017
business_scenario_id: MCP-CONFLUENCE-017
name: Search Confluence User
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-CONFLUENCE-017
tags:
  - regression
  - mcp
  - confluence
  - search-user
script_path: automation/playwright/tests/mcp/confluence/test_search_user.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Search Confluence User
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-CONFLUENCE-017 |
| Business Scenario Name | Search Confluence User |
| Coverage | Primary successful path for `mcp-confluence-search-user` |
...
Automates positive user search. Partial-match ranking, inactive users and privacy-restricted fields are excluded.
...
|---|---|
| User query | Read from secure non-secret configuration |
| Expected identifier | Read from configuration |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Invoke search-user. | The invocation succeeds. |
| 2 | Inspect returned users. | The expected identifier is present. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/confluence/test_search_user.py`
- Add traceability markers for `MCP-CONFLUENCE-A-017` and `MCP-CONFLUENCE-017`.
...
@pytest.mark.automation_id("MCP-CONFLUENCE-A-017")
@pytest.mark.business_scenario_id("MCP-CONFLUENCE-017")
def test_search_user(
    ...
    run_id = f"mcp-confluence-a-017-{int(time.time())}"
    test_data = confluence_test_data["search_user"]
    ...
    # Select or confirm `mcp-confluence-search-user`.
...
- pytest result associated with `MCP-CONFLUENCE-A-017`
...
- Execution metadata associated with `MCP-CONFLUENCE-017` and `MCP-CONFLUENCE-A-017`
