---
automation_test_id: MCP-GITHUB-A-031
business_scenario_id: MCP-GITHUB-031
name: List Files Changed in GitHub Pull Request
status: DRAFT
priority: HIGH
...
script_path: automation/playwright/tests/mcp/github/test_list_pull_request_files.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List Files Changed in GitHub Pull Request

## Objective

Verify automatically that an authorised caller can list changed files for an existing pull request with a known changed file, and that the expected file path is included.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-031 |
| Business Scenario Name | List Files Changed in GitHub Pull Request |
| Coverage | Primary successful path for `mcp-github-list-pull-request-files` |
...
Automates the primary successful path for `mcp-github-list-pull-request-files`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- The target an existing pull request with a known changed file exists and is accessible.
- The `mcp-github-list-pull-request-files` tool is available in the GitHub MCP server.
...
|---:|---|---|
| 1 | Submit the valid identifiers required to list changed files. | The request is accepted without validation or permission errors. |
| 2 | Retrieve the tool response. | A non-error object or collection is returned. |
| 3 | Inspect the returned data. | The expected file path is included. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_list_pull_request_files.py`
- Add traceability markers for `MCP-GITHUB-A-031` and `MCP-GITHUB-031`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-031")
@pytest.mark.business_scenario_id("MCP-GITHUB-031")
def test_list_pull_request_files(
    ...
    run_id = f"mcp-github-a-031-{int(time.time())}"
    test_data = github_test_data["list_pull_request_files"]
    ...
    # Select or confirm `mcp-github-list-pull-request-files`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the expected file path is included.
...
- pytest result associated with `MCP-GITHUB-A-031`
...
- Execution metadata associated with `MCP-GITHUB-031` and `MCP-GITHUB-A-031`
