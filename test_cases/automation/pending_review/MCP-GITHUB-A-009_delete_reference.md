---
automation_test_id: MCP-GITHUB-A-009
business_scenario_id: MCP-GITHUB-009
name: Delete Git Reference
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - delete-reference
script_path: automation/playwright/tests/mcp/github/test_delete_reference.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Delete Git Reference

## Objective

Verify automatically that an authorised caller can delete a disposable Git reference for an existing repository, and that the reference is no longer returned.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-009 |
| Business Scenario Name | Delete Git Reference |
| Coverage | Primary successful path for `mcp-github-delete-reference` |
...
Automates the primary successful path for `mcp-github-delete-reference`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_delete_reference.py`
- Add traceability markers for `MCP-GITHUB-A-009` and `MCP-GITHUB-009`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-009")
@pytest.mark.business_scenario_id("MCP-GITHUB-009")
def test_delete_reference(
    ...
    run_id = f"mcp-github-a-009-{int(time.time())}"
    test_data = github_test_data["delete_reference"]
    ...
    # Select or confirm `mcp-github-delete-reference`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the reference is no longer returned.
...
- pytest result associated with `MCP-GITHUB-A-009`
...
- Execution metadata associated with `MCP-GITHUB-009` and `MCP-GITHUB-A-009`
