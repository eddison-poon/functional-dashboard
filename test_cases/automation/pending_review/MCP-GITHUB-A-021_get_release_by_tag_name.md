---
automation_test_id: MCP-GITHUB-A-021
business_scenario_id: MCP-GITHUB-021
name: Retrieve GitHub Release by Tag
status: DRAFT
priority: HIGH
...
script_path: automation/playwright/tests/mcp/github/test_get_release_by_tag_name.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve GitHub Release by Tag

## Objective

Verify automatically that an authorised caller can retrieve a release by tag for an existing release tag, and that the returned release tag matches the submitted tag.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-021 |
| Business Scenario Name | Retrieve GitHub Release by Tag |
| Coverage | Primary successful path for `mcp-github-get-release-by-tag-name` |
...
Automates the primary successful path for `mcp-github-get-release-by-tag-name`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- The target an existing release tag exists and is accessible.
- The `mcp-github-get-release-by-tag-name` tool is available in the GitHub MCP server.
...
|---:|---|---|
| 1 | Submit the valid identifiers required to retrieve a release by tag. | The request is accepted without validation or permission errors. |
| 2 | Retrieve the tool response. | A non-error object or collection is returned. |
| 3 | Inspect the returned data. | The returned release tag matches the submitted tag. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_get_release_by_tag_name.py`
- Add traceability markers for `MCP-GITHUB-A-021` and `MCP-GITHUB-021`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-021")
@pytest.mark.business_scenario_id("MCP-GITHUB-021")
def test_get_release_by_tag_name(
    page: Page,
    ...
    run_id = f"mcp-github-a-021-{int(time.time())}"
    test_data = github_test_data["get_release_by_tag_name"]
    ...
    # Select or confirm `mcp-github-get-release-by-tag-name`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the returned release tag matches the submitted tag.
...
- pytest result associated with `MCP-GITHUB-A-021`
...
- Execution metadata associated with `MCP-GITHUB-021` and `MCP-GITHUB-A-021`
