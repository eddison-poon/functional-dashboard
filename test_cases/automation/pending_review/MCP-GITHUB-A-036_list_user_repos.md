---
automation_test_id: MCP-GITHUB-A-036
business_scenario_id: MCP-GITHUB-036
name: List GitHub User Repositories
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - list-user-repos
script_path: automation/playwright/tests/mcp/github/test_list_user_repos.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List GitHub User Repositories

## Objective

Verify automatically that an authorised caller can list repositories for a GitHub user with a known accessible repository, and that the expected repository is included.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-036 |
| Business Scenario Name | List GitHub User Repositories |
| Coverage | Primary successful path for `mcp-github-list-user-repos` |
...
Automates the primary successful path for `mcp-github-list-user-repos`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_list_user_repos.py`
- Add traceability markers for `MCP-GITHUB-A-036` and `MCP-GITHUB-036`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-036")
@pytest.mark.business_scenario_id("MCP-GITHUB-036")
def test_list_user_repos(
    ...
    run_id = f"mcp-github-a-036-{int(time.time())}"
    test_data = github_test_data["list_user_repos"]
    ...
    # Select or confirm `mcp-github-list-user-repos`.
...
- pytest result associated with `MCP-GITHUB-A-036`
...
- Execution metadata associated with `MCP-GITHUB-036` and `MCP-GITHUB-A-036`
