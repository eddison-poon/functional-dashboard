---
automation_test_id: MCP-JIRA-A-019
business_scenario_id: MCP-JIRA-019
name: Retrieve Existing Jira Issue
status: DRAFT
priority: HIGH
...
tags:
  ...
  - jira
  - get-issue
script_path: automation/playwright/tests/mcp/jira/test_get_issue.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Existing Jira Issue
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-JIRA-019 |
| Business Scenario Name | Retrieve Existing Jira Issue |
| Coverage | Primary successful path for `mcp-jira-jira-get-issue` |
...
Automates retrieval of one accessible existing issue. Expanded field sets, restricted fields and invalid references are excluded.
...
| Expected values | Capture from setup or configuration |
...
|---:|---|---|
| 1 | Ensure the get-issue tool is available. | The tool is listed and selectable. |
| 2 | Invoke get-issue with the configured issue reference. | The invocation succeeds and returns issue data. |
| 3 | Validate returned fields. | The key or ID, summary and issue type match expected values. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/jira/test_get_issue.py`
- Include markers or metadata for `MCP-JIRA-A-019` and `MCP-JIRA-019`.
...
@pytest.mark.mcp
@pytest.mark.jira
@pytest.mark.automation_id("MCP-JIRA-A-019")
@pytest.mark.business_scenario_id("MCP-JIRA-019")
def test_get_issue(page: Page) -> None:
    base_url = os.getenv("BASE_URL")
    if not base_url:
        pytest.fail("BASE_URL is required")

    unique_value = f"MCP-JIRA-A-019-{int(time.time())}"

    # Arrange
    page.goto(base_url)

    # Act
    # Open the configured agent and invoke `mcp-jira-jira-get-issue`.
...
- Delete setup data only when the issue was created by the test.

## Expected Execution Outputs

- pytest result associated with `MCP-JIRA-A-019`
...
- Execution metadata associated with `MCP-JIRA-019` and `MCP-JIRA-A-019`
