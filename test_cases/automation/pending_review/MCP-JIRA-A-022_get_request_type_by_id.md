---
automation_test_id: MCP-JIRA-A-022
business_scenario_id: MCP-JIRA-022
name: Retrieve Jira Service Request Type by ID
status: DRAFT
priority: HIGH
...
script_path: automation/playwright/tests/mcp/jira/test_get_request_type_by_id.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Jira Service Request Type by ID
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-JIRA-022 |
| Business Scenario Name | Retrieve Jira Service Request Type by ID |
| Coverage | Primary successful path for `mcp-jira-jira-get-request-type-by-id` |
...
Automates retrieval of one known request type. Invalid IDs, inaccessible service desks and field-schema completeness are excluded.
...
| Expected request type name | Read from test configuration |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Invoke get-request-type-by-id with the configured ID. | The invocation succeeds. |
| 2 | Inspect the returned object. | The ID matches the requested value. |
| 3 | Validate request-type metadata. | The expected request type name is returned. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/jira/test_get_request_type_by_id.py`
- Add traceability markers for `MCP-JIRA-A-022` and `MCP-JIRA-022`.
...
@pytest.mark.mcp
@pytest.mark.jira
@pytest.mark.automation_id("MCP-JIRA-A-022")
@pytest.mark.business_scenario_id("MCP-JIRA-022")
def test_get_request_type_by_id(
    page: Page,
    ...
    # Arrange
    page.goto(base_url)
    test_data = jira_test_data["get_request_type_by_id"]

    # Act
    # Open the configured agent.
    # Select or confirm `mcp-jira-jira-get-request-type-by-id`.
...
- pytest result associated with `MCP-JIRA-A-022`
...
- Execution metadata associated with `MCP-JIRA-022` and `MCP-JIRA-A-022`
