---
automation_test_id: MCP-JIRA-A-016
business_scenario_id: MCP-JIRA-016
name: Retrieve Comments from Jira Issue
status: DRAFT
priority: HIGH
...
tags:
  ...
  - jira
  - get-comments
script_path: automation/playwright/tests/mcp/jira/test_get_comments.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Comments from Jira Issue
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-JIRA-016 |
| Business Scenario Name | Retrieve Comments from Jira Issue |
| Coverage | Primary successful path for `mcp-jira-jira-get-comments` |
...
Automates retrieval of issue comments containing a known marker. Pagination, restricted comments and deleted comments are excluded.
...
| Expected marker | Create a unique comment during setup |
...
|---:|---|---|
| 1 | Prepare an issue containing a known comment. | The issue and comment are available. |
| 2 | Invoke get-comments using the issue reference. | The invocation succeeds and returns comments. |
| 3 | Inspect the returned collection. | The known comment marker is present. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/jira/test_get_comments.py`
- Include markers or metadata for `MCP-JIRA-A-016` and `MCP-JIRA-016`.
...
@pytest.mark.mcp
@pytest.mark.jira
@pytest.mark.automation_id("MCP-JIRA-A-016")
@pytest.mark.business_scenario_id("MCP-JIRA-016")
def test_get_comments(page: Page) -> None:
    base_url = os.getenv("BASE_URL")
    if not base_url:
        pytest.fail("BASE_URL is required")

    unique_value = f"MCP-JIRA-A-016-{int(time.time())}"

    # Arrange
    page.goto(base_url)

    # Act
    # Open the configured agent and invoke `mcp-jira-jira-get-comments`.
...
- Delete the setup comment or issue after verification.

## Expected Execution Outputs

- pytest result associated with `MCP-JIRA-A-016`
...
- Execution metadata associated with `MCP-JIRA-016` and `MCP-JIRA-A-016`
