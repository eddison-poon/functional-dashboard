---
automation_test_id: MCP-GITHUB-A-026
business_scenario_id: MCP-GITHUB-026
name: List Commits of GitHub Pull Request
status: DRAFT
priority: HIGH
...
script_path: automation/playwright/tests/mcp/github/test_list_commits_of_pull_request.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List Commits of GitHub Pull Request

## Objective

Verify automatically that an authorised caller can list pull request commits for an existing pull request, and that the expected commit SHA is included.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-026 |
| Business Scenario Name | List Commits of GitHub Pull Request |
| Coverage | Primary successful path for `mcp-github-list-commits-of-pull-request` |
...
Automates the primary successful path for `mcp-github-list-commits-of-pull-request`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- The target an existing pull request exists and is accessible.
- The `mcp-github-list-commits-of-pull-request` tool is available in the GitHub MCP server.
...
|---:|---|---|
| 1 | Submit the valid identifiers required to list pull request commits. | The request is accepted without validation or permission errors. |
| 2 | Retrieve the tool response. | A non-error object or collection is returned. |
| 3 | Inspect the returned data. | The expected commit sha is included. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_list_commits_of_pull_request.py`
- Add traceability markers for `MCP-GITHUB-A-026` and `MCP-GITHUB-026`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-026")
@pytest.mark.business_scenario_id("MCP-GITHUB-026")
def test_list_commits_of_pull_request(
    page: Page,
    ...
    run_id = f"mcp-github-a-026-{int(time.time())}"
    test_data = github_test_data["list_commits_of_pull_request"]
    ...
    # Select or confirm `mcp-github-list-commits-of-pull-request`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the expected commit SHA is included.
...
- pytest result associated with `MCP-GITHUB-A-026`
...
- Execution metadata associated with `MCP-GITHUB-026` and `MCP-GITHUB-A-026`
