---
automation_test_id: MCP-GITHUB-A-022
business_scenario_id: MCP-GITHUB-022
name: Retrieve GitHub Repository Details
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - get-repo-details
script_path: automation/playwright/tests/mcp/github/test_get_repo_details.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve GitHub Repository Details

## Objective

Verify automatically that an authorised caller can retrieve repository metadata for an accessible repository, and that the returned owner, name and default branch match expectations.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-022 |
| Business Scenario Name | Retrieve GitHub Repository Details |
| Coverage | Primary successful path for `mcp-github-get-repo-details` |
...
Automates the primary successful path for `mcp-github-get-repo-details`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_get_repo_details.py`
- Add traceability markers for `MCP-GITHUB-A-022` and `MCP-GITHUB-022`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-022")
@pytest.mark.business_scenario_id("MCP-GITHUB-022")
def test_get_repo_details(
    ...
    run_id = f"mcp-github-a-022-{int(time.time())}"
    test_data = github_test_data["get_repo_details"]
    ...
    # Select or confirm `mcp-github-get-repo-details`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the returned owner, name and default branch match expectations.
...
- pytest result associated with `MCP-GITHUB-A-022`
...
- Execution metadata associated with `MCP-GITHUB-022` and `MCP-GITHUB-A-022`
