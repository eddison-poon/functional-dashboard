---
automation_test_id: MCP-GITHUB-A-014
business_scenario_id: MCP-GITHUB-014
name: Retrieve GitHub Repository Content
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - get-content
script_path: automation/playwright/tests/mcp/github/test_get_content.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve GitHub Repository Content

## Objective

Verify automatically that an authorised caller can retrieve repository content for an existing file or directory path, and that the returned path and content metadata match the target.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-014 |
| Business Scenario Name | Retrieve GitHub Repository Content |
| Coverage | Primary successful path for `mcp-github-get-content` |
...
Automates the primary successful path for `mcp-github-get-content`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- The target an existing file or directory path exists and is accessible.
- The `mcp-github-get-content` tool is available in the GitHub MCP server.
...
|---:|---|---|
| 1 | Submit the valid identifiers required to retrieve repository content. | The request is accepted without validation or permission errors. |
| 2 | Retrieve the tool response. | A non-error object or collection is returned. |
| 3 | Inspect the returned data. | The returned path and content metadata match the target. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_get_content.py`
- Add traceability markers for `MCP-GITHUB-A-014` and `MCP-GITHUB-014`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-014")
@pytest.mark.business_scenario_id("MCP-GITHUB-014")
def test_get_content(
    ...
    run_id = f"mcp-github-a-014-{int(time.time())}"
    test_data = github_test_data["get_content"]
    ...
    # Select or confirm `mcp-github-get-content`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the returned path and content metadata match the target.
...
- pytest result associated with `MCP-GITHUB-A-014`
...
- Execution metadata associated with `MCP-GITHUB-014` and `MCP-GITHUB-A-014`
