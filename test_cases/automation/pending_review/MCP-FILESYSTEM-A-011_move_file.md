---
automation_test_id: MCP-FILESYSTEM-A-011
business_scenario_id: MCP-FILESYSTEM-011
name: Move File within Allowed Filesystem
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-FILESYSTEM-011
tags:
  - regression
  - mcp
  - filesystem
  - move-file
script_path: automation/playwright/tests/mcp/filesystem/test_move_file.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Move File within Allowed Filesystem
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-FILESYSTEM-011 |
| Business Scenario Name | Move File within Allowed Filesystem |
| Coverage | Primary successful path for `mcp-filesystem-move-file` |
...
Automates moving one disposable file between allowed locations. Overwrite behaviour, cross-device moves and permission failures are excluded.
...
|---|---|
| Source path | Create a disposable file |
| Destination directory | Create a disposable destination directory |
| Original content | Generate a unique known marker |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Create source file and destination directory. | Both setup paths exist. |
| 2 | Invoke move-file. | The invocation succeeds. |
| 3 | Invoke get-file-info for source and destination. | Source is missing and destination exists. |
| 4 | Invoke read-text-file for the destination. | The content matches the original marker. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/filesystem/test_move_file.py`
- Add traceability markers for `MCP-FILESYSTEM-A-011` and `MCP-FILESYSTEM-011`.
...
@pytest.mark.automation_id("MCP-FILESYSTEM-A-011")
@pytest.mark.business_scenario_id("MCP-FILESYSTEM-011")
def test_move_file(
    page: Page,
    ...
    run_id = f"mcp-filesystem-a-011-{int(time.time())}"
    disposable_path = PurePosixPath(test_root) / run_id
    test_data = filesystem_test_data["move_file"]
    ...
    # Select or confirm `mcp-filesystem-move-file`.
...
- Delete the destination file and disposable directories.

## Expected Execution Outputs

- pytest result associated with `MCP-FILESYSTEM-A-011`
...
- Execution metadata associated with `MCP-FILESYSTEM-011` and `MCP-FILESYSTEM-A-011`
