---
automation_test_id: MCP-FILESYSTEM-A-004
business_scenario_id: MCP-FILESYSTEM-004
name: Retrieve Recursive Directory Tree
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-FILESYSTEM-004
tags:
  - regression
  - mcp
  - filesystem
  - directory-tree
script_path: automation/playwright/tests/mcp/filesystem/test_directory_tree.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Recursive Directory Tree
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-FILESYSTEM-004 |
| Business Scenario Name | Retrieve Recursive Directory Tree |
| Coverage | Primary successful path for `mcp-filesystem-directory-tree` |
...
Automates recursive tree retrieval for a controlled directory. Very deep trees, symlink cycles and inaccessible descendants are excluded.
...
|---|---|
| Root path | Create a disposable nested directory structure during setup |
| Expected entries | Capture generated child directory and file names |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Create a root directory with one nested directory and file. | The expected hierarchy exists. |
| 2 | Invoke directory-tree for the root path. | The invocation succeeds and returns hierarchical data. |
| 3 | Inspect the tree. | All generated entries appear under the correct parents. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/filesystem/test_directory_tree.py`
- Add traceability markers for `MCP-FILESYSTEM-A-004` and `MCP-FILESYSTEM-004`.
...
@pytest.mark.automation_id("MCP-FILESYSTEM-A-004")
@pytest.mark.business_scenario_id("MCP-FILESYSTEM-004")
def test_directory_tree(
    ...
    run_id = f"mcp-filesystem-a-004-{int(time.time())}"
    disposable_path = PurePosixPath(test_root) / run_id
    test_data = filesystem_test_data["directory_tree"]
    ...
    # Select or confirm `mcp-filesystem-directory-tree`.
...
- Delete the disposable root directory and its contents.

## Expected Execution Outputs

- pytest result associated with `MCP-FILESYSTEM-A-004`
...
- Execution metadata associated with `MCP-FILESYSTEM-004` and `MCP-FILESYSTEM-A-004`
