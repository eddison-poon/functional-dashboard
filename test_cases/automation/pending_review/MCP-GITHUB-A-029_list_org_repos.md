---
automation_test_id: MCP-GITHUB-A-029
business_scenario_id: MCP-GITHUB-029
name: List GitHub Organization Repositories
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - list-org-repos
script_path: automation/playwright/tests/mcp/github/test_list_org_repos.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List GitHub Organization Repositories

## Objective

Verify automatically that an authorised caller can list repositories for an accessible organization containing a known repository, and that the expected repository is included.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-029 |
| Business Scenario Name | List GitHub Organization Repositories |
| Coverage | Primary successful path for `mcp-github-list-org-repos` |
...
Automates the primary successful path for `mcp-github-list-org-repos`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- The target an accessible organization containing a known repository exists and is accessible.
- The `mcp-github-list-org-repos` tool is available in the GitHub MCP server.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_list_org_repos.py`
- Add traceability markers for `MCP-GITHUB-A-029` and `MCP-GITHUB-029`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-029")
@pytest.mark.business_scenario_id("MCP-GITHUB-029")
def test_list_org_repos(
    ...
    run_id = f"mcp-github-a-029-{int(time.time())}"
    test_data = github_test_data["list_org_repos"]
    ...
    # Select or confirm `mcp-github-list-org-repos`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the expected repository is included.
...
- pytest result associated with `MCP-GITHUB-A-029`
...
- Execution metadata associated with `MCP-GITHUB-029` and `MCP-GITHUB-A-029`
