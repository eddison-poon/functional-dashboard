---
automation_test_id: MCP-GITHUB-A-037
business_scenario_id: MCP-GITHUB-037
...
tags:
  ...
  - github
  - merge-branch
script_path: automation/playwright/tests/mcp/github/test_merge_branch.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Merge GitHub Branch

## Objective

Verify automatically that an authorised caller can merge a source branch for a repository with compatible source and target branches, and that the target branch contains the source commit after the merge.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-037 |
| Business Scenario Name | Merge GitHub Branch |
| Coverage | Primary successful path for `mcp-github-merge-branch` |
...
Automates the primary successful path for `mcp-github-merge-branch`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_merge_branch.py`
- Add traceability markers for `MCP-GITHUB-A-037` and `MCP-GITHUB-037`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-037")
@pytest.mark.business_scenario_id("MCP-GITHUB-037")
def test_merge_branch(
    ...
    run_id = f"mcp-github-a-037-{int(time.time())}"
    test_data = github_test_data["merge_branch"]
    ...
    # Select or confirm `mcp-github-merge-branch`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the target branch contains the source commit after the merge.
...
- pytest result associated with `MCP-GITHUB-A-037`
...
- Execution metadata associated with `MCP-GITHUB-037` and `MCP-GITHUB-A-037`
