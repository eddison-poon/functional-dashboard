---
automation_test_id: MCP-JIRA-A-024
business_scenario_id: MCP-JIRA-024
name: Retrieve Request Types for Jira Service Desk
status: DRAFT
priority: HIGH
...
tags:
  ...
  - mcp
  - jira
  - get-request-types-by-service-desk-id
script_path: automation/playwright/tests/mcp/jira/test_get_request_types_by_service_desk_id.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Request Types for Jira Service Desk
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-JIRA-024 |
| Business Scenario Name | Retrieve Request Types for Jira Service Desk |
| Coverage | Primary successful path for `mcp-jira-jira-get-request-types-by-service-desk-id` |
...
Automates positive retrieval of request types by service desk. Pagination, inaccessible request types and empty desks are excluded.
...
|---|---|
| Service desk ID | Read from test configuration |
| Expected request type | Read from test configuration |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Invoke the tool using the configured service desk ID. | The invocation succeeds and returns request types. |
| 2 | Inspect returned request types. | The expected request type is present. |
| 3 | Validate desk relationship. | Returned data corresponds to the configured service desk. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/jira/test_get_request_types_by_service_desk_id.py`
- Add traceability markers for `MCP-JIRA-A-024` and `MCP-JIRA-024`.
...
@pytest.mark.mcp
@pytest.mark.jira
@pytest.mark.automation_id("MCP-JIRA-A-024")
@pytest.mark.business_scenario_id("MCP-JIRA-024")
def test_get_request_types_by_service_desk_id(
    page: Page,
    ...
    # Arrange
    page.goto(base_url)
    test_data = jira_test_data["get_request_types_by_service_desk_id"]

    # Act
    # Open the configured agent.
    # Select or confirm `mcp-jira-jira-get-request-types-by-service-desk-id`.
...
- pytest result associated with `MCP-JIRA-A-024`
...
- Execution metadata associated with `MCP-JIRA-024` and `MCP-JIRA-A-024`
