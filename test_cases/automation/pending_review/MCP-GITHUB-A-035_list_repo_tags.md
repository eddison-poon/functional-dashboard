---
automation_test_id: MCP-GITHUB-A-035
business_scenario_id: MCP-GITHUB-035
name: List GitHub Repository Tags
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - list-repo-tags
script_path: automation/playwright/tests/mcp/github/test_list_repo_tags.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List GitHub Repository Tags

## Objective

Verify automatically that an authorised caller can list tags for an existing repository with a known tag, and that the expected tag is included.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-035 |
| Business Scenario Name | List GitHub Repository Tags |
| Coverage | Primary successful path for `mcp-github-list-repo-tags` |
...
Automates the primary successful path for `mcp-github-list-repo-tags`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_list_repo_tags.py`
- Add traceability markers for `MCP-GITHUB-A-035` and `MCP-GITHUB-035`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-035")
@pytest.mark.business_scenario_id("MCP-GITHUB-035")
def test_list_repo_tags(
    ...
    run_id = f"mcp-github-a-035-{int(time.time())}"
    test_data = github_test_data["list_repo_tags"]
...
- pytest result associated with `MCP-GITHUB-A-035`
...
- Execution metadata associated with `MCP-GITHUB-035` and `MCP-GITHUB-A-035`
