---
automation_test_id: MCP-JIRA-A-021
business_scenario_id: MCP-JIRA-021
name: Retrieve Jira Issue Link Types
status: DRAFT
priority: HIGH
...
tags:
  ...
  - jira
  - get-link-types
script_path: automation/playwright/tests/mcp/jira/test_get_link_types.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Jira Issue Link Types
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-JIRA-021 |
| Business Scenario Name | Retrieve Jira Issue Link Types |
| Coverage | Primary successful path for `mcp-jira-jira-get-link-types` |
...
Automates positive retrieval of issue-link metadata. Administrative creation or modification of link types is excluded.
...
|---|---|
| Expected link type | Read from test configuration |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Invoke get-link-types. | The invocation succeeds and returns a collection. |
| 2 | Inspect link-type metadata. | The configured expected type is present. |
| 3 | Validate required fields. | The returned type includes an identifier or name and supported direction labels. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/jira/test_get_link_types.py`
- Add traceability markers for `MCP-JIRA-A-021` and `MCP-JIRA-021`.
...
@pytest.mark.mcp
@pytest.mark.jira
@pytest.mark.automation_id("MCP-JIRA-A-021")
@pytest.mark.business_scenario_id("MCP-JIRA-021")
def test_get_link_types(
    page: Page,
    ...
    # Arrange
    page.goto(base_url)
    test_data = jira_test_data["get_link_types"]

    # Act
    # Open the configured agent.
    # Select or confirm `mcp-jira-jira-get-link-types`.
...
- pytest result associated with `MCP-JIRA-A-021`
...
- Execution metadata associated with `MCP-JIRA-021` and `MCP-JIRA-A-021`
