---
automation_test_id: MCP-JIRA-A-014
business_scenario_id: MCP-JIRA-014
name: Delete Jira Issue Worklog
status: DRAFT
priority: HIGH
...
tags:
  ...
  - jira
  - delete-worklog
script_path: automation/playwright/tests/mcp/jira/test_delete_worklog.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Delete Jira Issue Worklog
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-JIRA-014 |
| Business Scenario Name | Delete Jira Issue Worklog |
| Coverage | Primary successful path for `mcp-jira-jira-delete-worklog` |
...
Automates deletion of a disposable worklog. Ownership restrictions, invalid IDs and permission failures are excluded.
...
| Worklog ID | Create a disposable worklog during setup and retain its ID |
...
|---:|---|---|
| 1 | Prepare a disposable worklog. | A valid worklog reference is available. |
| 2 | Invoke delete-worklog. | The invocation succeeds. |
| 3 | Inspect the issue worklogs. | The deleted worklog is absent. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/jira/test_delete_worklog.py`
- Include markers or metadata for `MCP-JIRA-A-014` and `MCP-JIRA-014`.
...
@pytest.mark.mcp
@pytest.mark.jira
@pytest.mark.automation_id("MCP-JIRA-A-014")
@pytest.mark.business_scenario_id("MCP-JIRA-014")
def test_delete_worklog(page: Page) -> None:
    base_url = os.getenv("BASE_URL")
    if not base_url:
        pytest.fail("BASE_URL is required")

    unique_value = f"MCP-JIRA-A-014-{int(time.time())}"

    # Arrange
    page.goto(base_url)

    # Act
    # Open the configured agent and invoke `mcp-jira-jira-delete-worklog`.
...
- pytest result associated with `MCP-JIRA-A-014`
...
- Execution metadata associated with `MCP-JIRA-014` and `MCP-JIRA-A-014`
