---
automation_test_id: MCP-FILESYSTEM-A-010
business_scenario_id: MCP-FILESYSTEM-010
name: List Directory Entries with Sizes
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-FILESYSTEM-010
tags:
  - regression
  - mcp
  - filesystem
  - list-directory-with-sizes
script_path: automation/playwright/tests/mcp/filesystem/test_list_directory_with_sizes.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List Directory Entries with Sizes
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-FILESYSTEM-010 |
| Business Scenario Name | List Directory Entries with Sizes |
| Coverage | Primary successful path for `mcp-filesystem-list-directory-with-sizes` |
...
Automates size-aware listing for controlled files. Directory-size aggregation and platform-specific allocation sizes are excluded.
...
| File content | Generate known byte content |
| Expected size | Calculate from the encoded content |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Create a child file with known byte content. | The expected size is known. |
| 2 | Invoke list-directory-with-sizes. | The invocation succeeds and returns entries with sizes. |
| 3 | Validate the child file entry. | The filename and reported size match expectations. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/filesystem/test_list_directory_with_sizes.py`
- Add traceability markers for `MCP-FILESYSTEM-A-010` and `MCP-FILESYSTEM-010`.
...
@pytest.mark.automation_id("MCP-FILESYSTEM-A-010")
@pytest.mark.business_scenario_id("MCP-FILESYSTEM-010")
def test_list_directory_with_sizes(
    page: Page,
    ...
    run_id = f"mcp-filesystem-a-010-{int(time.time())}"
    disposable_path = PurePosixPath(test_root) / run_id
    test_data = filesystem_test_data["list_directory_with_sizes"]
    ...
    # Select or confirm `mcp-filesystem-list-directory-with-sizes`.
...
- Delete the disposable directory and file.

## Expected Execution Outputs

- pytest result associated with `MCP-FILESYSTEM-A-010`
...
- Execution metadata associated with `MCP-FILESYSTEM-010` and `MCP-FILESYSTEM-A-010`
