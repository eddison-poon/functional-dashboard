---
automation_test_id: MCP-FILESYSTEM-A-008
business_scenario_id: MCP-FILESYSTEM-008
name: List Allowed Filesystem Directories
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-FILESYSTEM-008
tags:
  - regression
  - mcp
  - filesystem
  - list-allowed-directories
script_path: automation/playwright/tests/mcp/filesystem/test_list_allowed_directories.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List Allowed Filesystem Directories
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-FILESYSTEM-008 |
| Business Scenario Name | List Allowed Filesystem Directories |
| Coverage | Primary successful path for `mcp-filesystem-list-allowed-directories` |
...
Automates positive retrieval of configured allowed roots. Administrative policy changes and denied-path testing are excluded.
...
|---|---|
| Expected allowed root | Read from secure environment configuration |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Invoke list-allowed-directories. | The invocation succeeds and returns allowed roots. |
| 2 | Normalise returned paths for comparison. | Path formatting is made consistent without changing meaning. |
| 3 | Validate results. | The configured expected root is present. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/filesystem/test_list_allowed_directories.py`
- Add traceability markers for `MCP-FILESYSTEM-A-008` and `MCP-FILESYSTEM-008`.
...
@pytest.mark.automation_id("MCP-FILESYSTEM-A-008")
@pytest.mark.business_scenario_id("MCP-FILESYSTEM-008")
def test_list_allowed_directories(
    page: Page,
    ...
    run_id = f"mcp-filesystem-a-008-{int(time.time())}"
    disposable_path = PurePosixPath(test_root) / run_id
    test_data = filesystem_test_data["list_allowed_directories"]
    ...
    # Select or confirm `mcp-filesystem-list-allowed-directories`.
...
- pytest result associated with `MCP-FILESYSTEM-A-008`
...
- Execution metadata associated with `MCP-FILESYSTEM-008` and `MCP-FILESYSTEM-A-008`
