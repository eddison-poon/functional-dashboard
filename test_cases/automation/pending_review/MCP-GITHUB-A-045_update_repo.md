---
automation_test_id: MCP-GITHUB-A-045
business_scenario_id: MCP-GITHUB-045
name: Update GitHub Repository Settings
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - update-repo
script_path: automation/playwright/tests/mcp/github/test_update_repo.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Update GitHub Repository Settings

## Objective

Verify automatically that an authorised caller can update supported repository settings for a disposable or approved test repository, and that the changed repository settings match the submitted values.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-045 |
| Business Scenario Name | Update GitHub Repository Settings |
| Coverage | Primary successful path for `mcp-github-update-repo` |
...
Automates the primary successful path for `mcp-github-update-repo`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_update_repo.py`
- Add traceability markers for `MCP-GITHUB-A-045` and `MCP-GITHUB-045`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-045")
@pytest.mark.business_scenario_id("MCP-GITHUB-045")
def test_update_repo(
    ...
    run_id = f"mcp-github-a-045-{int(time.time())}"
    test_data = github_test_data["update_repo"]
    ...
    # Select or confirm `mcp-github-update-repo`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the changed repository settings match the submitted values.
...
- pytest result associated with `MCP-GITHUB-A-045`
...
- Execution metadata associated with `MCP-GITHUB-045` and `MCP-GITHUB-A-045`
