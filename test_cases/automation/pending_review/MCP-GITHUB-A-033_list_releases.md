---
automation_test_id: MCP-GITHUB-A-033
business_scenario_id: MCP-GITHUB-033
name: List GitHub Repository Releases
status: DRAFT
priority: HIGH
...
tags:
  ...
  - github
  - list-releases
script_path: automation/playwright/tests/mcp/github/test_list_releases.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List GitHub Repository Releases

## Objective

Verify automatically that an authorised caller can list releases for a repository with a known release, and that the expected release tag is included.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-033 |
| Business Scenario Name | List GitHub Repository Releases |
| Coverage | Primary successful path for `mcp-github-list-releases` |
...
Automates the primary successful path for `mcp-github-list-releases`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- The target a repository with a known release exists and is accessible.
- The `mcp-github-list-releases` tool is available in the GitHub MCP server.
...
|---:|---|---|
| 1 | Submit the valid identifiers required to list releases. | The request is accepted without validation or permission errors. |
| 2 | Retrieve the tool response. | A non-error object or collection is returned. |
| 3 | Inspect the returned data. | The expected release tag is included. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_list_releases.py`
- Add traceability markers for `MCP-GITHUB-A-033` and `MCP-GITHUB-033`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-033")
@pytest.mark.business_scenario_id("MCP-GITHUB-033")
def test_list_releases(
    ...
    run_id = f"mcp-github-a-033-{int(time.time())}"
    test_data = github_test_data["list_releases"]
    ...
    # Select or confirm `mcp-github-list-releases`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the expected release tag is included.
...
- pytest result associated with `MCP-GITHUB-A-033`
...
- Execution metadata associated with `MCP-GITHUB-033` and `MCP-GITHUB-A-033`
