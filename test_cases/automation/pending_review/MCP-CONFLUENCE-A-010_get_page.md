---
automation_test_id: MCP-CONFLUENCE-A-010
business_scenario_id: MCP-CONFLUENCE-010
name: Retrieve Existing Confluence Page
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-CONFLUENCE-010
tags:
  - regression
  - mcp
  - confluence
  - get-page
script_path: automation/playwright/tests/mcp/confluence/test_get_page.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Existing Confluence Page
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-CONFLUENCE-010 |
| Business Scenario Name | Retrieve Existing Confluence Page |
| Coverage | Primary successful path for `mcp-confluence-get-page` |
...
Automates retrieval of one accessible page. Version history and restricted pages are excluded.
...
|---:|---|---|
| 1 | Invoke get-page. | The invocation succeeds. |
| 2 | Validate identity. | The returned page reference matches. |
| 3 | Validate content. | Title and content marker match. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/confluence/test_get_page.py`
- Add traceability markers for `MCP-CONFLUENCE-A-010` and `MCP-CONFLUENCE-010`.
...
@pytest.mark.automation_id("MCP-CONFLUENCE-A-010")
@pytest.mark.business_scenario_id("MCP-CONFLUENCE-010")
def test_get_page(
    page: Page,
    ...
    run_id = f"mcp-confluence-a-010-{int(time.time())}"
    test_data = confluence_test_data["get_page"]
    ...
    # Select or confirm `mcp-confluence-get-page`.
...
- pytest result associated with `MCP-CONFLUENCE-A-010`
...
- Execution metadata associated with `MCP-CONFLUENCE-010` and `MCP-CONFLUENCE-A-010`
