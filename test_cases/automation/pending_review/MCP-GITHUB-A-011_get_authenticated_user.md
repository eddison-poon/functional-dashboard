---
automation_test_id: MCP-GITHUB-A-011
business_scenario_id: MCP-GITHUB-011
name: Retrieve Authenticated GitHub User
status: DRAFT
priority: HIGH
...
script_path: automation/playwright/tests/mcp/github/test_get_authenticated_user.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Retrieve Authenticated GitHub User

## Objective

Verify automatically that an authorised caller can retrieve user information for the current authenticated GitHub session, and that the returned login or account identifier matches configured expectations.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-011 |
| Business Scenario Name | Retrieve Authenticated GitHub User |
| Coverage | Primary successful path for `mcp-github-get-authenticated-user` |
...
Automates the primary successful path for `mcp-github-get-authenticated-user`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_get_authenticated_user.py`
- Add traceability markers for `MCP-GITHUB-A-011` and `MCP-GITHUB-011`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-011")
@pytest.mark.business_scenario_id("MCP-GITHUB-011")
def test_get_authenticated_user(
    ...
    run_id = f"mcp-github-a-011-{int(time.time())}"
    test_data = github_test_data["get_authenticated_user"]
    ...
    # Select or confirm `mcp-github-get-authenticated-user`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the returned login or account identifier matches configured expectations.
...
- pytest result associated with `MCP-GITHUB-A-011`
...
- Execution metadata associated with `MCP-GITHUB-011` and `MCP-GITHUB-A-011`
