---
automation_test_id: MCP-CONFLUENCE-A-019
business_scenario_id: MCP-CONFLUENCE-019
name: Update Existing Confluence Space
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-CONFLUENCE-019
tags:
  - regression
  - mcp
  - confluence
  - update-space
script_path: automation/playwright/tests/mcp/confluence/test_update_space.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Update Existing Confluence Space
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-CONFLUENCE-019 |
| Business Scenario Name | Update Existing Confluence Space |
| Coverage | Primary successful path for `mcp-confluence-update-space` |
...
Automates updating supported space metadata. Immutable fields and permission failures are excluded.
...
|---|---|
| Space reference | Create during setup or read from configuration |
| Updated description | Generate `MCP-CONFLUENCE-A-019 ${timestamp}` |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Prepare the target test space. | A valid reference is available. |
| 2 | Invoke update-space. | The invocation succeeds. |
| 3 | Invoke get-space. | The generated updated value is returned. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/confluence/test_update_space.py`
- Add traceability markers for `MCP-CONFLUENCE-A-019` and `MCP-CONFLUENCE-019`.
...
@pytest.mark.automation_id("MCP-CONFLUENCE-A-019")
@pytest.mark.business_scenario_id("MCP-CONFLUENCE-019")
def test_update_space(
    ...
    run_id = f"mcp-confluence-a-019-{int(time.time())}"
    test_data = confluence_test_data["update_space"]
    ...
    # Select or confirm `mcp-confluence-update-space`.
...
- Restore the original value or delete the disposable space.

## Expected Execution Outputs

- pytest result associated with `MCP-CONFLUENCE-A-019`
...
- Execution metadata associated with `MCP-CONFLUENCE-019` and `MCP-CONFLUENCE-A-019`
