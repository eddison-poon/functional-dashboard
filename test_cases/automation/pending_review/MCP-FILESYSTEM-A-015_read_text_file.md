---
automation_test_id: MCP-FILESYSTEM-A-015
business_scenario_id: MCP-FILESYSTEM-015
name: Read Text File from Allowed Path
status: DRAFT
priority: HIGH
...
requirement_ids:
  - REQ-MCP-FILESYSTEM-015
tags:
  - regression
  - mcp
  - filesystem
  - read-text-file
script_path: automation/playwright/tests/mcp/filesystem/test_read_text_file.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Read Text File from Allowed Path
...
| Field | Value |
|---|---|
| Business Scenario ID | MCP-FILESYSTEM-015 |
| Business Scenario Name | Read Text File from Allowed Path |
| Coverage | Primary successful path for `mcp-filesystem-read-text-file` |
...
Automates reading one UTF-8 text file. Unsupported encodings, binary files and very large content are excluded.
...
|---|---|
| Target file | Create a disposable UTF-8 file |
| Expected text | Generate `MCP-FILESYSTEM-A-015-${timestamp}` |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Create a UTF-8 text file with generated content. | The file exists. |
| 2 | Invoke read-text-file. | The invocation succeeds and returns text. |
| 3 | Compare returned text. | It exactly matches the generated source content. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/filesystem/test_read_text_file.py`
- Add traceability markers for `MCP-FILESYSTEM-A-015` and `MCP-FILESYSTEM-015`.
...
@pytest.mark.automation_id("MCP-FILESYSTEM-A-015")
@pytest.mark.business_scenario_id("MCP-FILESYSTEM-015")
def test_read_text_file(
    ...
    run_id = f"mcp-filesystem-a-015-{int(time.time())}"
    disposable_path = PurePosixPath(test_root) / run_id
    test_data = filesystem_test_data["read_text_file"]
    ...
    # Select or confirm `mcp-filesystem-read-text-file`.
...
- Delete the disposable text file.

## Expected Execution Outputs

- pytest result associated with `MCP-FILESYSTEM-A-015`
...
- Execution metadata associated with `MCP-FILESYSTEM-015` and `MCP-FILESYSTEM-A-015`
