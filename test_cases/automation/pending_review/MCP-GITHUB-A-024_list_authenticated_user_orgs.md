---
automation_test_id: MCP-GITHUB-A-024
business_scenario_id: MCP-GITHUB-024
name: List Authenticated User GitHub Organizations
status: DRAFT
priority: HIGH
...
tags:
  ...
  - mcp
  - github
  - list-authenticated-user-orgs
script_path: automation/playwright/tests/mcp/github/test_list_authenticated_user_orgs.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: List Authenticated User GitHub Organizations

## Objective

Verify automatically that an authorised caller can list organizations for the authenticated GitHub user, and that the expected organization is included.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-024 |
| Business Scenario Name | List Authenticated User GitHub Organizations |
| Coverage | Primary successful path for `mcp-github-list-authenticated-user-orgs` |
...
Automates the primary successful path for `mcp-github-list-authenticated-user-orgs`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_list_authenticated_user_orgs.py`
- Add traceability markers for `MCP-GITHUB-A-024` and `MCP-GITHUB-024`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-024")
@pytest.mark.business_scenario_id("MCP-GITHUB-024")
def test_list_authenticated_user_orgs(
    page: Page,
    ...
    run_id = f"mcp-github-a-024-{int(time.time())}"
    test_data = github_test_data["list_authenticated_user_orgs"]
    ...
    # Select or confirm `mcp-github-list-authenticated-user-orgs`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the expected organization is included.
...
- pytest result associated with `MCP-GITHUB-A-024`
...
- Execution metadata associated with `MCP-GITHUB-024` and `MCP-GITHUB-A-024`
