---
automation_test_id: ARK-SANDBOX-A-009
business_scenario_id: ARK-SANDBOX-009
name: Upload File to Sandbox
status: DRAFT
priority: HIGH
...
tags:
  ...
  - ark-sandbox
  - upload-file
script_path: automation/playwright/tests/sandbox/ark/test_upload_file.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Upload File to Sandbox
...
| Field | Value |
|---|---|
| Business Scenario ID | ARK-SANDBOX-009 |
| Business Scenario Name | Upload File to Sandbox |
| Coverage | Primary successful path for `ark-sandbox-upload-file` |
...
Automates upload of one small permitted non-sensitive file. Large files, restricted paths, overwrite conflicts and transfer interruption are excluded.
...
| Local fixture | Generate `ark-sandbox-a-009-${timestamp}.txt` |
| File content | Generate a unique non-sensitive marker |
| Remote path | Use a disposable writable path inside the sandbox |

## Automated Flow and Assertions

| Step | Automated Action | Required Assertion |
|---:|---|---|
| 1 | Prepare a running sandbox and local fixture file. | Both are available. |
| 2 | Invoke upload-file. | The invocation succeeds. |
| 3 | Invoke download-file for the remote path. | The remote file is returned. |
| 4 | Compare content. | The uploaded and downloaded content match. |
...
- Test runner: pytest
- Script path: `automation/playwright/tests/sandbox/ark/test_upload_file.py`
- Add traceability markers for `ARK-SANDBOX-A-009` and `ARK-SANDBOX-009`.
...
@pytest.mark.automation_id("ARK-SANDBOX-A-009")
@pytest.mark.business_scenario_id("ARK-SANDBOX-009")
def test_upload_file(
    ...
    run_id = f"ark-sandbox-a-009-{int(time.time())}"
    test_data = ark_sandbox_test_data["upload_file"]

    fixture_path = Path(test_data["fixture_path"])
    if not fixture_path.is_file():
        pytest.fail(f"Upload fixture does not exist: {fixture_path}")
    ...
    # Select or confirm `ark-sandbox-upload-file`.
...
- pytest result associated with `ARK-SANDBOX-A-009`
...
- Execution metadata associated with `ARK-SANDBOX-009` and `ARK-SANDBOX-A-009`
