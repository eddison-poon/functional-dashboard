---
automation_test_id: MCP-GITHUB-A-010
business_scenario_id: MCP-GITHUB-010
...
tags:
  ...
  - github
  - delete-repo
script_path: automation/playwright/tests/mcp/github/test_delete_repo.py
jira_id:
owner: Functional Test Automation Team
---

# Automation Test Definition: Delete GitHub Repository

## Objective

Verify automatically that an authorised caller can delete a disposable repository for an authenticated user or organization, and that the repository is no longer retrievable.

## Parent Business Scenario

| Field | Value |
|---|---|
| Business Scenario ID | MCP-GITHUB-010 |
| Business Scenario Name | Delete GitHub Repository |
| Coverage | Primary successful path for `mcp-github-delete-repo` |
...
Automates the primary successful path for `mcp-github-delete-repo`. Invalid identifiers, insufficient permissions, rate limits, concurrency conflicts and unsupported options are excluded.
...
- Test runner: pytest
- Script path: `automation/playwright/tests/mcp/github/test_delete_repo.py`
- Add traceability markers for `MCP-GITHUB-A-010` and `MCP-GITHUB-010`.
...
@pytest.mark.mcp
@pytest.mark.github
@pytest.mark.automation_id("MCP-GITHUB-A-010")
@pytest.mark.business_scenario_id("MCP-GITHUB-010")
def test_delete_repo(
    ...
    run_id = f"mcp-github-a-010-{int(time.time())}"
    test_data = github_test_data["delete_repo"]
    ...
    # Select or confirm `mcp-github-delete-repo`.
    # Populate the exact MCP request using `test_data` and `run_id`.
    # Submit the invocation.

    # Assert
    # Verify invocation success.
    # Verify that the repository is no longer retrievable.
...
- pytest result associated with `MCP-GITHUB-A-010`
...
- Execution metadata associated with `MCP-GITHUB-010` and `MCP-GITHUB-A-010`
